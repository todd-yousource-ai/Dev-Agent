# TRD-4-Multi-Agent-Coordination-Crafted

_Source: `TRD-4-Multi-Agent-Coordination-Crafted.docx` — extracted 2026-03-24 21:19 UTC_
...
Storage | GitHub repository: crafted-docs/BUILD_LEDGER.json on default branch
...
// crafted-docs/BUILD_LEDGER.json
{
    "schema_version":   2,
...
"pr_entries": {
    "1": {
        ...
        "branch":       "crafted/build/todd/payments-pr001-add-transaction-validator",
        "prd_id":       "PRD-001",
        ...
        // Journal reference
        "journal_path": "crafted-docs/journals/PR-001.md",
...
Build Ledger | crafted-docs/BUILD_LEDGER.json | Stage 2 — initialise() | Any engineer: register, claim, heartbeat, mark_done, add_note
PR Journal | crafted-docs/journals/PR-{NNN:03d}.md | Stages 7–8 — mark_pr_done() | Only the engineer who completed the PR
    ...
    for attempt in range(MAX_WRITE_RETRIES):
        try:
            ...
            self._github.commit_file(
                ...
                path=LEDGER_PATH,
                content=content,
                message=f"crafted-ledger[{self._engineer_id}]: {data.get('updated_by','?')}",
...
LEDGER_PATH   = "crafted-docs/BUILD_LEDGER.json"
JOURNALS_PATH = "crafted-docs/journals"
...
def mark_pr_done(
    ...
    pr_num:         int,
    pr_url:         str,
    journal_summary: str,  # Written to crafted-docs/journals/PR-NNN.md
) -> None:
    """
    ...
    try:
        self._github.commit_file(
            ...
            message=f"crafted-ledger[{self._engineer_id}]: journal PR-{pr_num:03d}",
        )
    except Exception as e:
...
# When crafted-docs/BUILD_LEDGER.json is in the changed files:
# TRD-5 sends an XPC message to the Swift shell (TRD-1):
{
    ...
    "payload":   {"path": "crafted-docs/BUILD_LEDGER.json"}
}
...
# crafted-docs/journals/PR-{NNN:03d}.md
# Written by Stage 7–8 after PR completion.
# Plain markdown — readable by engineers and parseable by the agent.

# Example: crafted-docs/journals/PR-007.md
---
PR: 7
Title: PR007 Add idempotency key expiry
Branch: crafted/build/todd/payments-pr007-add-idempotency-key-expiry
Engineer: todd-gould
PRD: PRD-002
...
OQ-04 | crafted-docs/ directory naming: current design stores ledger and journals in crafted-docs/. This may conflict with TRD documentation also stored in crafted-docs/. Recommendation: renamed to crafted-docs/; superseded by brand rename crafted/ to separate agent coordination files from project docs. | Engineering | Sprint 1
...
journal_path | str|null | Yes | "crafted-docs/journals/PR-NNN.md"
