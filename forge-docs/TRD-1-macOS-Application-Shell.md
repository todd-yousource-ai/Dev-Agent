...
_Source: `TRD-1-macOS-Application-Shell.docx` — extracted 2026-03-20 15:19 UTC_
...
actor KeychainManager {
    ...
    private let service = "ai.yousource.forgeagent"
...
let socketPath = FileManager.default
    .temporaryDirectory
    .appendingPathComponent("forgeagent-\(ProcessInfo.processInfo.processIdentifier).sock")
    .path

// Passed to Python via environment:
env["FORGE_XPC_SOCKET"] = socketPath

// Python backend connects as client after startup:
// import socket; s = socket.socket(socket.AF_UNIX); s.connect(FORGE_XPC_SOCKET)
...
env["FORGE_XPC_NONCE"] = nonce

// Handshake protocol:
// 1. Python connects to socket
// 2. Python sends:  { "type": "handshake", "nonce": "<FORGE_XPC_NONCE value>" }
...
File system access | No | Scoped to ~/Library/Application Support/ForgeAgent/ by code convention, not by OS enforcement
...
actor BackendProcess {
    ...
    func launch() async throws {
        ...
        let socketPath = NSTemporaryDirectory() +
            "forgeagent-\(ProcessInfo.processInfo.processIdentifier).sock"
        ...
        p.environment   = [
            ...
            "FORGE_WORKSPACE":  appSupportPath,
            "FORGE_LOG_DIR":    logsPath,
...
Appcast URL | https://updates.yousource.ai/forgeagent/appcast.xml | Served over HTTPS only
...
// Signing a release:
$ ./bin/sign_update ForgeAgent-1.1.0.zip
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
  <channel>
    <title>Forge Agent Updates</title>
    <link>https://updates.yousource.ai/forgeagent/appcast.xml</link>
    <item>
      ...
      <sparkle:releaseNotesLink>
          https://updates.yousource.ai/forgeagent/release-1.1.0.html
      </sparkle:releaseNotesLink>
      <pubDate>Wed, 19 Mar 2026 12:00:00 +0000</pubDate>
      <enclosure
          url="https://updates.yousource.ai/forgeagent/ForgeAgent-1.1.0.zip"
...
extension Logger {
    static let auth    = Logger(subsystem:"ai.yousource.forgeagent", category:"auth")
    static let keychain = Logger(subsystem:"ai.yousource.forgeagent", category:"keychain")
    static let xpc     = Logger(subsystem:"ai.yousource.forgeagent", category:"xpc")
    static let process = Logger(subsystem:"ai.yousource.forgeagent", category:"process")
    static let settings = Logger(subsystem:"ai.yousource.forgeagent", category:"settings")
    static let docimport = Logger(subsystem:"ai.yousource.forgeagent", category:"docimport")
    static let ui      = Logger(subsystem:"ai.yousource.forgeagent", category:"ui")
    static let update  = Logger(subsystem:"ai.yousource.forgeagent", category:"update")
}
...
$ xcrun atos -arch arm64 -o ForgeAgent.app.dSYM/Contents/Resources/DWARF/ForgeAgent \
...
keychain-access-groups | $(AppIdentifierPrefix)ai.yousource.forgeagent | Required for Keychain access
