# TRD-9-Mac-CI-Runner-Crafted

_Source: `TRD-9-Mac-CI-Runner-Crafted.docx` — extracted 2026-03-23 19:18 UTC_
...
This document specifies the complete technical requirements for the Mac CI Runner Infrastructure — a self-hosted GitHub Actions runner on the developer's MacBook that gives the Crafted a real macOS build environment.
...
crafted-ci-macos.yml — the complete macOS GitHub Actions workflow
...
crafted-ci.yml update — job routing between Ubuntu and Mac runners
...
Separate workflow file | crafted-ci-macos.yml, not merged into crafted-ci.yml | Separate files for separate runners prevents a Mac runner offline state from blocking Python CI. Each workflow is independently required.
...
# Crafted.entitlements — required for code signing
# Location: Crafted/Crafted.entitlements in Xcode project
...
<dict>
    ...
    <array>
        <string>$(AppIdentifierPrefix)ai.yousource.crafted</string>
    </array>
</dict>
</plist>
...
# 6. crafted-ci-macos.yml — Workflow Overview

## 6.1 Trigger Specification

name: Crafted CI — macOS
...
on:
  push:
    branches:
      ...
      - "crafted/build/**"
    paths:
      - "Crafted/**"        # Swift source
      - "CraftedTests/**"   # Swift tests
    ...
    paths-ignore:
      ...
      - "**.md"
      - "**.docx"
      - "src/**"               # Python source — handled by crafted-ci.yml
      - "tests/**"             # Python tests
  pull_request:
    branches:
      - main
...
- name: Verify build output
  run: |
    if [ ! -d "$GITHUB_WORKSPACE/build/Release/Crafted.app" ]; then
      echo "ERROR: Crafted.app not found after build"
      exit 1
    fi
    echo "Build output: $(du -sh build/Release/Crafted.app | cut -f1)"
    # Print bundle structure for debugging
    find build/Release/Crafted.app -type f | head -30
...
class XPCIntegrationTest: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        let appPath = ProcessInfo.processInfo.environment["APP_PATH"]
            ?? "\(FileManager.default.currentDirectoryPath)/build/Release/Crafted.app"
...
- name: Run XPC integration test
  run: |
    ...
    | xcbeautify --renderer github-actions
  env:
    APP_PATH: ${{ github.workspace }}/build/Release/Crafted.app
  timeout-minutes: 5
...
- name: Inject Python bundle into .app
  run: |
    RESOURCES="build/Release/Crafted.app/Contents/Resources"
...
- name: Notarize
  run: |
    ...
    ditto -c -k --keepParent Crafted.app Crafted.zip

    # Submit to Apple Notary Service
    xcrun notarytool submit Crafted.zip \
...
- name: Staple
  run: xcrun stapler staple Crafted.app

- name: Verify notarization
  run: |
    xcrun stapler validate Crafted.app
    spctl --assess --type exec Crafted.app
...
- name: Create .dmg
  run: |
    create-dmg \
      ...
      --window-size 800 400 \
      --icon-size 100 \
      --icon "Crafted.app" 200 190 \
      --hide-extension "Crafted.app" \
      --app-drop-link 600 185 \
      "Crafted-${{ github.ref_name }}.dmg" \
      "Crafted.app"

- name: Notarize .dmg
  run: |
    xcrun notarytool submit "Crafted-${{ github.ref_name }}.dmg" \
      --key ~/.appstoreconnect/private_keys/AuthKey_${{ secrets.ASC_API_KEY_ID }}.p8 \
      --key-id ${{ secrets.ASC_API_KEY_ID }} \
      --issuer ${{ secrets.ASC_API_KEY_ISSUER }} \
      --wait --timeout 15m
    xcrun stapler staple "Crafted-${{ github.ref_name }}.dmg"
...
forge-agent-unsigned-{run_id} | Every PR and push | Crafted.app (unsigned) | 7 days | Download and run locally to inspect UI before merge
forge-agent-signed-{run_id} | Every push to main and forge-agent/* branches | Crafted.app (signed, not notarized) | 14 days | Working app for internal testing without full notarization wait
...
Crafted.app.dSYM-{version} | Tags (v*.*.*) | Debug symbols for crash symbolication | 90 days | Required to symbolicate crash reports from that version
...
- name: Upload unsigned app
  ...
  with:
    ...
    path: build/Release/Crafted.app
    retention-days: 7
...
- name: Upload dSYMs
  ...
  uses: actions/upload-artifact@v4
  with:
    name: Crafted.app.dSYM-${{ github.ref_name }}
    path: build/Release/Crafted.app.dSYM
    retention-days: 90
...
0 3 * * 0 xcrun simctl delete unavailable 2>/dev/null; \
          rm -rf ~/Library/Developer/Xcode/DerivedData/Crafted-* 2>/dev/null; \
          rm -rf ~/actions-runner/_work/_temp 2>/dev/null

# Runs every Sunday at 3am.
# Crafted-* DerivedData: recreated on next CI run (cache warmup)
...
# 15. crafted-ci.yml Update — Job Routing

## 15.1 Routing Logic

# crafted-ci.yml handles Python. crafted-ci-macos.yml handles Swift.
# They run independently. Both must pass for a PR to merge into main.

# Python CI triggers on Python file changes:
# crafted-ci.yml paths:
#   - "src/**"
#   - "tests/**"
#   - "requirements.txt"
#   - ".github/workflows/crafted-ci.yml"
#   paths-ignore: Crafted/**, *.xcodeproj/**

# macOS CI triggers on Swift file changes:
# crafted-ci-macos.yml paths:
#   - "Crafted/**"
#   - "CraftedTests/**"
#   - "*.xcodeproj/**"
...
#   "Forge CI — Python / test"         (from crafted-ci.yml)
#   "Forge CI — macOS / unit-test"     (from crafted-ci-macos.yml)
...
Signing produces valid binary | codesign --verify --deep --strict Crafted.app | Exit code 0
...
# Appendix A: Complete crafted-ci-macos.yml

name: Crafted CI — macOS

on:
  push:
    branches: [main, "crafted/build/**"]
    paths:
      - "Crafted/**"
      - "CraftedTests/**"
      - "XPCIntegrationTests/**"
      ...
      - "requirements.txt"
      - ".github/workflows/crafted-ci-macos.yml"
    paths-ignore: ["prds/**", "crafted-docs/**", "**.md", "**.docx", "src/**", "tests/**"]
  pull_request:
    branches: [main]
    paths: ["Crafted/**", "CraftedTests/**", "XPCIntegrationTests/**",
...
jobs:
  ...
  build:
    ...
    steps:
      ...
      - name: Upload unsigned app
        ...
        with:
          name: forge-agent-unsigned-${{ github.run_id }}
          path: ${{ env.BUILD_DIR }}/Release/Crafted.app
          retention-days: 7
  ...
  sign:
    ...
    steps:
      ...
      - name: Sign app
        run: |
          codesign --force --deep --sign "${{ secrets.APPLE_SIGNING_IDENTITY }}" \
            --entitlements Crafted/Crafted.entitlements \
            --options runtime \
            unsigned/Crafted.app
          codesign --verify --deep --strict unsigned/Crafted.app
      - name: Lock Keychain
        if: always()
        run: security lock-keychain ~/Library/Keychains/login.keychain-db
      - name: Upload signed app
  ...
  notarize:
    ...
    steps:
      ...
      - name: Notarize
        run: |
          ditto -c -k --keepParent signed/Crafted.app Crafted.zip
          xcrun notarytool submit Crafted.zip \
            --key ~/.appstoreconnect/private_keys/AuthKey_${{ secrets.ASC_API_KEY_ID }}.p8 \
            --key-id ${{ secrets.ASC_API_KEY_ID }} \
            --issuer ${{ secrets.ASC_API_KEY_ISSUER }} \
            --wait --timeout 15m
          xcrun stapler staple signed/Crafted.app
      - name: Create DMG
        run: |
          ...
          create-dmg \
            ...
            --icon "Crafted.app" 200 190 \
            --app-drop-link 600 185 \
            "Crafted-${VERSION}.dmg" "signed/Crafted.app"
          xcrun notarytool submit "Crafted-${VERSION}.dmg" \
            --key ~/.appstoreconnect/private_keys/AuthKey_${{ secrets.ASC_API_KEY_ID }}.p8 \
            --key-id ${{ secrets.ASC_API_KEY_ID }} \
            --issuer ${{ secrets.ASC_API_KEY_ISSUER }} \
            --wait --timeout 15m
          xcrun stapler staple "Crafted-${VERSION}.dmg"
      - name: Upload DMG
        uses: actions/upload-artifact@v4
        with:
          name: Crafted-${{ github.ref_name }}.dmg
          path: Crafted-${{ github.ref_name }}.dmg
          retention-days: 90
      - name: Cleanup
        if: always()
        run: rm -f ~/.appstoreconnect/private_keys/AuthKey_*.p8
...
<dict>
    <key>Label</key>
    <string>actions.runner.{owner}-{repo}.macbook-crafted-runner</string>
