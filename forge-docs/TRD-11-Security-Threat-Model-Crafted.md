# TRD-11-Security-Threat-Model-Crafted

_Source: `TRD-11-Security-Threat-Model-Crafted.docx` — extracted 2026-03-23 18:42 UTC_
...
This document defines the security threat model for the Crafted — a system that reads external documents, calls LLM APIs with those documents as context, generates code, commits it to GitHub, and runs it in CI on a machine with code signing credentials. Each of those operations is an attack surface.
...
Agent only ingests comments on its own crafted/* PRs | PRReviewIngester filter — TRD-5 S13 | MEDIUM — reduces exposure surface significantly
...
3. PR workflow contains:
   - run: security find-generic-password -a Crafted -w
...
SEC-CRED-05 | CI Keychain locked immediately after signing step, even on failure (if: always()). | crafted-ci-macos.yml review
SEC-CRED-06 | ASC API key .p8 file deleted immediately after notarization, even on failure. | crafted-ci-macos.yml review
...
CI runner security | Review crafted-ci-macos.yml for new secret exposure | No new secrets printed. Fork protection still enabled. Workflow changes reviewed.
Entitlements review | Verify Crafted.entitlements unchanged from last release | No new capabilities added without explicit security review.
