...
_Source: `TRD-7-TRD-Development-Workflow.docx` — extracted 2026-03-21 18:58 UTC_
...
Major additions in v2.0: - Technical literacy detection — Phase 0 (new) - Adaptive language layer — all phases translate to/from business language for non-technical users (§3) - Business-to-technical translation engine — how business answers become technical specifications (§4) - Non-technical question register — complete rewrite of Phase 2 for business-language users (§6) - Pipeline handoff — explicit connection from TRD completion to deployment (§17) - Operator mode labels — FOUNDER mode vs ENGINEER mode (§3.2)

Sections unchanged from v1.0: §7 (Phase 3 — Boundary Definition), §8 (Phase 4 — Deep Dive taxonomy), §9 (Phase 5 — Gap Detection), §10 (Phase 6 — Outline Review), §11 (Phase 7 — PRODUCT_CONTEXT.md), §12 (Phase 8 — TRD Generation), §13 (Iterative Refinement), §14 (Question Generation Protocol), §15 (Completion Signals), §18 (Session Persistence).
...
The workflow owns: - Phase 0: Technical literacy detection (new in v2.0) - Eight structured phases from product vision through TRD generation - Adaptive language layer — all questions and responses in the operator’s register - Business-to-technical translation — converting business answers to technical specifications - Dynamic question generation — one question at a time, adapted to what has been said - Question taxonomy — coverage map ensuring all TRD domains are addressed - Completion signals, gap detection, outline review - PRODUCT_CONTEXT.md synthesis - Pipeline handoff to /prd start (new in v2.0) - Session persistence with full resume support
...
Mode is stored on TRDSession and affects: - All question generation prompts (DEEP_DIVE_SYSTEM, GAP_DETECTION_SYSTEM) - Outline presentation format - TRD review gate language - Completion signal thresholds (FOUNDER mode has slightly lower thresholds — more “I don’t know” answers are acceptable and expected)
...
### 5.3 Mode Confirmation

Once mode is set, the agent confirms with an appropriate opening:
...
After Phase 2 in FOUNDER mode, the agent synthesizes the architecture sketch internally using the translation layer. The output format is identical to ENGINEER mode — the same architecture_sketch dict, the same ASCII diagram. The user sees only:

Here's what I understand about how your product works technically. 
I'll show you a simplified picture:

[business-language description of the architecture in 3-4 sentences]
...
FOUNDER mode adaptation: TRD boundaries are presented to the user as “components” not “TRDs.” The naming is in plain language.
...
SESSION COMPLETE — {N} TRDs generated
  ...
  PRODUCT_CONTEXT.md: forge-docs/PRODUCT_CONTEXT.md
  Total cost: ${cost:.2f}
...
For FOUNDER mode sessions, after the completion message, the agent automatically offers to proceed:

PIPELINE_OFFER_GATE = {
    "gate_type": "pipeline_start",
    "title":     "Start building your application?",
    "body": (
        "Your specifications are complete. The agent can begin "
        "building your application now. This will:\n"
        "  - Write all the code for each component\n"
        "  - Run automated tests\n"
        "  - Deploy your application to the cloud\n\n"
        "You can watch the progress and approve each component "
        "before it's built."
    ),
    "options": ["Start building", "I'll start later with /prd start"],
}

If the user selects “Start building”, the agent calls _handle_prd("start") internally, loading the just-generated TRDs as context. The user never types /prd start.

### 17.4 Cost and Time Transparency

Both modes receive a cost and time estimate at session start. This sets expectations correctly — a 4-hour investment before building begins is different from a 30-minute vibe-coding session.
...
2.0 | 2026-03-20 | YouSource.ai | Adaptive language layer: Phase 0 (technical literacy detection), FOUNDER mode, business-to-technical translation engine, FOUNDER-mode Phase 2 question register, pipeline handoff to build system, automatic /prd start offer, cost and time transparency
...
CONSULTANT mode is detected in Phase 0 by explicit signal, not inference. The agent does not try to detect consulting context from response vocabulary — it asks directly.
...
CONSULTANT mode signals:

CONSULTANT_SIGNALS = [
    "consultant", "consulting", "client", "on behalf",
    "service provider", "agency", "working with a client",
    "my client", "customer", "for a customer",
    "professional services", "systems integrator",
]

One or more signals → CONSULTANT mode confirmed.
...
BAD:  "Can you describe the authentication and authorization 
       requirements for the system, including how different 
       user roles will interact with the platform?"
...
CONSULTANT mode produces two distinct artifacts at session end.

Layer 1 — Client Summary (business-readable)

A one-page document the consultant can leave with the client, email immediately after the meeting, or use as a sign-off document before the build begins.

CLIENT_SUMMARY_SYSTEM = """
Generate a one-page client summary document.

This document is for a business stakeholder, not a developer.
It must be readable by someone with no technical background.

Sections:
1. What We're Building (2-3 sentences — plain language)
2. Who It's For (user types in plain language)
3. What It Does (5-7 bullet points — features, not specs)
4. What It Doesn't Do (2-3 explicit exclusions — sets expectations)
5. How It's Secured (2-3 sentences — non-technical security summary)
6. Deployment (one sentence — where it runs, who manages it)
7. Next Steps (what happens after client approves this document)

Style rules:
- No acronyms without explanation
- No technical terms (API, database, backend, etc.)
- Every sentence must be understandable by a non-technical executive
- Length: 400-600 words maximum
- Tone: confident and specific, not vague

This document will be used for client sign-off before build begins.
"""
...
CONSULTANT mode adds a critical field to the session: the deployment target. This determines what the build pipeline produces at the end — not just code, but a packaged, deployable artifact for the client’s environment.
...
The deployment target is determined in Phase 2 (Architecture Discovery) and drives: - The build pipeline’s packaging step - The CI workflow (Windows runner vs Mac runner vs Linux vs cloud) - The security model applied (desktop app security vs cloud service security) - The Forge runtime enforcement profile — each deployment target has a different trust model

CONSULTANT mode deployment question:

Does your client need this to run on their own computers and 
servers — inside their building or on their own cloud account — 
or is a hosted web service acceptable?

This single question determines the deployment target class. The agent maps the answer:
...
CONSULTANT mode includes a dedicated security pass not present in FOUNDER or ENGINEER mode. Enterprise clients have compliance requirements that must be captured before the build begins — not discovered during security review.

SECURITY_PASS_QUESTIONS = [
    # Asked in order, stopped when sufficient for the deployment context
    {
        "question": "Does your client operate under any regulatory "
                    "requirements — for example healthcare data rules, "
                    "financial regulations, or government standards?",
        "maps_to": "compliance_framework",
        "examples": "HIPAA, SOC 2, FedRAMP, PCI-DSS, ISO 27001",
    },
    {
        "question": "Does the application handle any sensitive data — "
                    "patient records, financial information, personal "
                    "employee data, or confidential business information?",
        "maps_to": "data_classification",
    },
    {
        "question": "Does your client's IT policy require that data "
                    "never leaves their own infrastructure?",
        "maps_to": "data_residency",
    },
    {
        "question": "Will this application need to connect to any of "
                    "your client's existing internal systems — like their "
                    "HR system, ERP, or internal database?",
        "maps_to": "internal_integrations",
    },
    {
        "question": "Does your client have a single sign-on system "
                    "their employees already use — like Microsoft Active "
                    "Directory, Okta, or Google Workspace?",
        "maps_to": "identity_provider",
    },
]

Answers are captured in trd_notes under a security_compliance domain and drive the Security Threat Model TRD (TRD-11) generation. Compliance frameworks detected automatically apply corresponding controls:

Framework detected | Controls applied
HIPAA | PHI encryption at rest and in transit, audit log, BAA language in summary
...
/trd pause     — Pause the session mid-meeting. Saves state immediately.
               ...
               Use when client needs a break or the meeting runs over.

/trd summary   — Show a running summary of what has been captured so far.
               Formatted for the consultant, not the client.
               Useful for confirming understanding mid-meeting before 
               moving to the next topic.

The /trd summary output in CONSULTANT mode:

═══ SESSION SUMMARY ═══════════════════════════════════════
Client: {client_name or "unnamed"}
Phase: {current_phase}
...
Captured so far:
  ...
  Compliance: {compliance_framework or "none identified"}

  Components identified:
    {list of TRD boundaries proposed so far}

  Open questions still to cover:
    {list of domains not yet completed}
══════════════════════════════════════════════════════════
...
1. Client Summary document (.docx) Generated first. Formatted for business stakeholders. Includes a sign-off section at the bottom (“Approved by: _______________ Date: _______________”).

2. Technical TRD set Same as FOUNDER mode — full specification committed to GitHub.

3. Consultant Handoff Card A card shown in the agent stream for the consultant’s use only:

═══ CONSULTANT HANDOFF ═════════════════════════════════════
Client summary: {docx_path}
  → Share this with the client for approval before build starts

Technical specification: {github_url}
  → {N} TRDs committed, ready for build pipeline

Deployment target: {deployment_target}
Compliance profile: {compliance_framework or "standard"}
Estimated build time: {hours} hours
Estimated build cost: ${min}–${max}

To start building after client approval:
  /prd start

To share the client summary:
  /trd export client-summary
══════════════════════════════════════════════════════════
