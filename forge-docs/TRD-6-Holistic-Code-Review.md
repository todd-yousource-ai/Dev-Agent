...
_Source: `TRD-6-Holistic-Code-Review.docx` — extracted 2026-03-21 18:58 UTC_
...
Phase 3: REPORT
  ...
  ├── Commit report to: forge-docs/reviews/{branch_slug}-{timestamp}.md
...
Phase 6: PR CREATION
  For each lens with fixes (or combined if single-PR mode):
    ...
    ▼
Phase 7: MANIFEST
  ├── Write ReviewManifest to forge-docs/reviews/{branch_slug}-manifest.json
...
Always excluded | — | *.docx, *.pdf, *.png, *.jpg, forge-docs/, prds/, .forge/, forge-agent/
...
# Path: forge-docs/reviews/{branch_slug}-{timestamp}.md
# Example: forge-docs/reviews/main-20260319-143022.md

REVIEWS_PATH = "forge-docs/reviews"
...
# ─────────────────────────────────────────────────────
# forge-docs/reviews/main-20260319-143022.md
# ─────────────────────────────────────────────────────
...
---
*Generated by Consensus Dev Agent — Holistic Code Review*
...
def commit_report(self, session: ReviewSession, report_md: str) -> None:
    ...
    try:
        self._github.commit_file(
            ...
            path=path,
            content=report_md,
            message=f"forge-review[{session.engineer_id}]: holistic review report",
        )
...
# Branch name: forge-review/{engineer_id}/{branch_slug}-{session_short_id}
# Example: forge-review/todd-gould/main-a1b2c3d4
...
Output the complete fixed file."""
    ...
    try:
        github.commit_file(
            ...
            message=f"forge-review[{session.engineer_id}]: fix {lens_id} issues in {path}",
        )
    except Exception as e:
...
Per-lens (default) | Multiple lenses with fixes | forge-review: {lens} fixes for {branch} — {N} issues | Operator can merge lenses independently
Single combined PR | Operator selects "single PR" at Gate 3, or only one lens has fixes | forge-review: holistic fixes for {branch} — {N} issues | Single merge — simpler for small codebases
...
{nr_section}
---
_Generated by Consensus Dev Agent — Holistic Code Review_
...
# Stored at: forge-docs/reviews/{branch_slug}-manifest.json
    ...
    "review_history": [
        {
            ...
            "issues_fixed": 61,
            "cost_usd":     18.40,
            "report_path":  "forge-docs/reviews/main-20260319-143022.md",
...
should_include() | 100% | Auto-exclude patterns match correctly; scope_dirs filter works; forge-docs/ always excluded; vendor/ always excluded
